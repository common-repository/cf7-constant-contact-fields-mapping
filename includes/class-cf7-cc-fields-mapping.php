...
class Cf7_CC_Fields_Mapping_ConstantContact_ContactPostRequest
{
    ...
    public function build( WPCF7_Submission $submission )
    {
        ...
        if ( !($this->first_name || $this->last_name) and isset( $posted_data['your-name'] ) ) {
        ...
        if ( isset( $posted_data['your-birthday-month'] ) and isset( $posted_data['your-birthday-day'] ) ) {
            $this->set_birthday( $posted_data['your-birthday-month'], $posted_data['your-birthday-day'] );
        ...
        $this->add_street_address(
            ( isset( $posted_data['your-address-street'] ) ? $posted_data['your-address-street'] : '' ),
            ( isset( $posted_data['your-address-city'] ) ? $posted_data['your-address-city'] : '' ),
            ( isset( $posted_data['your-address-state'] ) ? $posted_data['your-address-state'] : '' ),
            ( isset( $posted_data['your-address-postal-code'] ) ? $posted_data['your-address-postal-code'] : '' ),
            ( isset( $posted_data['your-address-country'] ) ? $posted_data['your-address-country'] : '' )
        );
        $service_option = (array) WPCF7::get_option( 'constant_contact' );
        $contact_lists = ( isset( $service_option['contact_lists'] ) ? $service_option['contact_lists'] : array() );
        $contact_form = $submission->get_contact_form();
        $fields_data = get_post_meta( $contact_form->id(), 'cf7_cc_fields_mapping_settings', true );
        foreach ( (array) $fields_data as $key => $value ) {
            switch ( $key ) {
                case 'first_name':
                    if ( isset( $posted_data[$value] ) ) {
                        $this->set_first_name( $posted_data[$value] );
                    }
                    break;
                case 'last_name':
                    if ( isset( $posted_data[$value] ) ) {
                        $this->set_last_name( $posted_data[$value] );
                    }
                    break;
                case 'first_name_last_name':
                    
                    if ( !($this->first_name || $this->last_name) and isset( $posted_data[$value] ) ) {
                        $your_name = preg_split( '/[\\s]+/', $posted_data[$value], 2 );
                        $this->set_first_name( array_shift( $your_name ) );
                        $this->set_last_name( array_shift( $your_name ) );
                    }
                ...
                case 'email_address':
                    if ( isset( $posted_data[$value] ) ) {
                        $this->set_email_address( $posted_data[$value], 'implicit' );
                    }
                    break;
                case 'job_title':
                    if ( isset( $posted_data[$value] ) ) {
                        $this->set_job_title( $posted_data[$value] );
                    }
                    break;
                case 'company_name':
                    if ( isset( $posted_data[$value] ) ) {
                        $this->set_company_name( $posted_data[$value] );
                    }
                    break;
                case 'birthday_month':
                    $birthday_month = $posted_data[$value];
                    break;
                case 'birthday_day':
                    $birthday_day = $posted_data[$value];
                    break;
                case 'birthday_month_day':
                    $birthday_month_day = $posted_data[$value];
                    break;
                case 'anniversary':
                    if ( isset( $posted_data[$value] ) ) {
                        $this->set_anniversary( $posted_data[$value] );
                    }
                    break;
                case 'phone_numbers':
                    if ( isset( $posted_data[$value] ) ) {
                        $this->add_phone_number( $posted_data[$value] );
                    }
                    break;
                case 'street':
                    if ( isset( $posted_data[$value] ) ) {
                        $street = $posted_data[$value];
                    }
                    break;
                case 'city':
                    if ( isset( $posted_data[$value] ) ) {
                        $city = $posted_data[$value];
                    }
                    break;
                case 'state':
                    if ( isset( $posted_data[$value] ) ) {
                        $state = $posted_data[$value];
                    }
                    break;
                case 'postal_code':
                    if ( isset( $posted_data[$value] ) ) {
                        $postal_code = $posted_data[$value];
                    }
                    break;
                case 'country':
                    if ( isset( $posted_data[$value] ) ) {
                        $country = $posted_data[$value];
                    }
                    break;
                default:
                    $unnonefield = '';
        ...
        if ( isset( $birthday_month ) and isset( $birthday_day ) ) {
            $this->set_birthday( $birthday_month, $birthday_day );
        } elseif ( isset( $birthday_month_day ) ) {
            $date = trim( $birthday_month_day );
            if ( preg_match( '/^(\\d{4})-(\\d{2})-(\\d{2})$/', $date, $matches ) ) {
                $this->set_birthday( $matches[2], $matches[3] );
            }
        }
    ...
    {
        return $this->create_source && ($this->email_address || $this->first_name || $this->last_name);
    }
    ...
    {
        if ( !wpcf7_is_email( $address ) or 80 < $this->strlen( $address ) ) {
            return false;
        }
    ...
    public function set_first_name( $first_name )
    {
        ...
        if ( empty($first_name) or 50 < $this->strlen( $first_name ) ) {
            return false;
        }
    ...
    public function set_last_name( $last_name )
    {
        ...
        if ( empty($last_name) or 50 < $this->strlen( $last_name ) ) {
            return false;
        }
    ...
    public function set_job_title( $job_title )
    {
        ...
        if ( empty($job_title) or 50 < $this->strlen( $job_title ) ) {
            return false;
        }
    ...
    public function set_company_name( $company_name )
    {
        ...
        if ( empty($company_name) or 50 < $this->strlen( $company_name ) ) {
            return false;
        }
    ...
    public function set_birthday( $month, $day )
    {
        $month = (int) $month;
        $day = (int) $day;
        if ( $month < 1 || 12 < $month or $day < 1 || 31 < $day ) {
            return false;
        }
    ...
    {
        $pattern = sprintf( '#^(%s)$#', implode( '|', array(
            '\\d{1,2}/\\d{1,2}/\\d{4}',
            '\\d{4}/\\d{1,2}/\\d{1,2}',
            '\\d{4}-\\d{1,2}-\\d{1,2}',
            '\\d{1,2}-\\d{1,2}-\\d{4}'
    ...
    public function add_custom_field( $custom_field_id, $value )
    {
        ...
        $value = trim( $value );
        if ( 25 <= count( $this->custom_fields ) or !preg_match( $uuid_pattern, $custom_field_id ) or 255 < $this->strlen( $value ) ) {
            return false;
        }
    ...
    public function add_phone_number( $phone_number, $kind = 'home' )
    {
        ...
        if ( 2 <= count( $this->phone_numbers ) or !wpcf7_is_tel( $phone_number ) or 25 < $this->strlen( $phone_number ) or !in_array( $kind, array( 'home', 'work', 'other' ) ) ) {
            return false;
        }
    ...
    public function add_street_address(
        $street,
        $city,
        $state,
        $postal_code,
    ...
    )
    {
        ...
        if ( !($street || $city || $state || $postal_code || $country) or 1 <= count( $this->street_addresses ) or !in_array( $kind, array( 'home', 'work', 'other' ) ) or 255 < $this->strlen( $street ) or 50 < $this->strlen( $city ) or 50 < $this->strlen( $state ) or 50 < $this->strlen( $postal_code ) or 50 < $this->strlen( $country ) ) {
            return false;
        }
    ...
    {
        $uuid_pattern = '/^[0-9a-f-]+$/i';
        if ( 50 <= count( $this->list_memberships ) or !preg_match( $uuid_pattern, $list_id ) ) {
            return false;
        }
